...
from app.schemas.interview_question import InterviewQuestionResponse, InterviewQuestionCreate, InterviewQuestionUpdate, InterviewQuestionWithDetails
...
class InterviewQuestionDAO(BaseDAO[InterviewQuestion, InterviewQuestionResponse, InterviewQuestionCreate, InterviewQuestionUpdate]):
    ...
    def get_with_details(self, db: Session, id: int) -> Optional[InterviewQuestionWithDetails]:
        """Get an interview question with question details."""
        interview_question = db.query(self.model).options(
            joinedload(self.model.question)
        ).filter(self.model.id == id).first()
        
        if interview_question:
            return InterviewQuestionWithDetails.from_model(interview_question)
        return None
    ...
    def mark_as_asked(self, db: Session, id: int) -> Optional[InterviewQuestionResponse]:
        ...
        interview_question = db.query(self.model).filter(self.model.id == id).first()
        if interview_question:
            interview_question.status = InterviewQuestionStatus.ASKED
            interview_question.asked_at = datetime.now(timezone.utc)
            db.commit()
            db.refresh(interview_question)
            return InterviewQuestionResponse.from_model(interview_question)
        return None
    ...
    def mark_as_answered(self, db: Session, id: int, *, answer: str, ai_analysis: Optional[dict] = None) -> Optional[InterviewQuestionResponse]:
        ...
        interview_question = db.query(self.model).filter(self.model.id == id).first()
        if interview_question:
            interview_question.status = InterviewQuestionStatus.ANSWERED
            interview_question.candidate_answer = answer
            interview_question.answered_at = datetime.now(timezone.utc)
            if ai_analysis:
                interview_question.ai_analysis = ai_analysis
            db.commit()
            db.refresh(interview_question)
            return InterviewQuestionResponse.from_model(interview_question)
        return None
    ...
    def mark_as_skipped(self, db: Session, id: int) -> Optional[InterviewQuestionResponse]:
        ...
        interview_question = db.query(self.model).filter(self.model.id == id).first()
        if interview_question:
            interview_question.status = InterviewQuestionStatus.SKIPPED
            db.commit()
            db.refresh(interview_question)
            return InterviewQuestionResponse.from_model(interview_question)
        return None
    ...
    def add_follow_up_questions(self, db: Session, id: int, *, follow_ups: dict) -> Optional[InterviewQuestionResponse]:
        ...
        interview_question = db.query(self.model).filter(self.model.id == id).first()
        if interview_question:
            interview_question.follow_up_questions = follow_ups
            db.commit()
            db.refresh(interview_question)
            return InterviewQuestionResponse.from_model(interview_question)
        return None

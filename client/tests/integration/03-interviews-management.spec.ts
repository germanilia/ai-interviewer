...
test.describe('Interview Management', () => {
  ...
  test.describe('Interview Monitoring', () => {
    test('should display in-progress interviews for monitoring', async () => {
      await interviewsPage.navigateTo();
      await interviewsPage.waitForInterviewsToLoad();
      
      // Filter to in-progress interviews
      await interviewsPage.filterByStatus('in_progress');
      
      const inProgressCount = await interviewsPage.getInterviewCount();
      if (inProgressCount > 0) {
        // Verify monitor button is available
        const firstRow = interviewsPage.interviewRows.first();
        await expect(firstRow.locator('[data-testid="monitor-btn"]')).toBeVisible();
      }
    });

    test('should open monitoring view for in-progress interview', async () => {
      await interviewsPage.navigateTo();
      await interviewsPage.waitForInterviewsToLoad();
      ...
      if (inProgressCount > 0) {
        ...
        await interviewsPage.monitorInterview(0);
        
        // Verify monitoring view opens
    ...
    test('should display interview progress correctly', async () => {
      await interviewsPage.navigateTo();
      await interviewsPage.waitForInterviewsToLoad();
      ...
      if (inProgressCount > 0) {
        ...
        await expect(interviewsPage.progressBar).toBeVisible();
        await expect(interviewsPage.progressText).toBeVisible();
        
        // Progress text should show format like "3 of 10 questions"
        const progressText = await interviewsPage.progressText.textContent();
        expect(progressText).toMatch(/\d+ of \d+ questions?/);
        
        // Progress bar should have a value
        const progressValue = await interviewsPage.progressBar.getAttribute('value');
        expect(parseInt(progressValue || '0')).toBeGreaterThanOrEqual(0);
      }
    });

    test('should show real-time candidate responses', async () => {
      await interviewsPage.navigateTo();
      await interviewsPage.waitForInterviewsToLoad();
      ...
      if (inProgressCount > 0) {
        ...
        // Verify response and analysis areas
        await expect(interviewsPage.candidateResponseArea).toBeVisible();
        await expect(interviewsPage.aiAnalysisArea).toBeVisible();
        
        // Verify intervention controls
        await expect(interviewsPage.interventionButton).toBeVisible();
        await expect(interviewsPage.addNotesButton).toBeVisible();
      }
    });
  });
